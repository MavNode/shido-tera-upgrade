...
TARGET_HEIGHT=28000000

echo "Waiting for block height $TARGET_HEIGHT before starting upgrade..."

while true; do
    CURRENT_HEIGHT=$(shidod status 2>/dev/null | grep -o '"latest_block_height":"[0-9]*"' | grep -o '[0-9]*')

    if [[ -n "$CURRENT_HEIGHT" && "$CURRENT_HEIGHT" -ge "$TARGET_HEIGHT" ]]; then
        echo "Target height reached: $CURRENT_HEIGHT"
        break
    fi
